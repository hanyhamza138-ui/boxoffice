...
export async function importEngine(file) {
  ...
  for (
    ...
    const item = {
      ...
      /*
        ...
        لذلك سنعتبرها matched فقط
        عندما تكون غير موجودة أصلًا.
      */
    ...
    if (invalidVersion) {
      ...
      try {
        ...
        if (
          ...
          /* ---------------------------------------
             Version Exists in File
             But NOT in Database

             مهم جدًا:
             لا نربطها بنسخة خاطئة.
             ولا نعتبرها Matched.
          --------------------------------------- */
        ...
        /* ---------------------------------------
           Version Matching Error

           لا نوقف الملف كله.
           لكن لا نعتبر Version مطابقة.
        --------------------------------------- */
  ...
  /* -----------------------------------------------
     Statistics:
     Movie / Cinema / Version
  ----------------------------------------------- */

  const unmatchedMovieRows =
    result.filter(
      (row) =>
        !row.matchedMovie
    ).length;

  const unmatchedCinemaRows =
    result.filter(
      (row) =>
        !row.matchedCinema
    ).length;

  const unmatchedVersionRows =
    result.filter(
      (row) =>
        !row.matchedVersion
    ).length;
